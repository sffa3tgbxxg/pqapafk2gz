...
class ApiErrorsRequest extends FormRequest
{
    ...
    public function filters(): ApiRequestsErrors
    {
        $dto = new ApiRequestsErrors();
        $dto->dateFrom = Carbon::parse($this->input('from'))->format('Y-m-d H:i:s');
        $dto->dateTo = Carbon::parse($this->input('to'))->format('Y-m-d H:i:s');
        $dto->invoiceId = $this->integer('invoice_id', null);
        $dto->exchangerId = $this->integer('exchanger_id', null);
